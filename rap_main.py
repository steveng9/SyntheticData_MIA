...
for HH in range(10):
    ...
    kway_attrs = dataset.randomKway(num_kways=args.workload, k=args.k, seed=None)
    ...
    args.categorical_consistency = True
    ...
    queries_used = rap.train(D, kway_attrs, key)
    overall_queries.append(queries_used)

def dump_artifact(artifact, name):
    pickle_file = open(f"/Users/golobs/Documents/GradSchool/Thesis/experiment_artifacts/{name}", 'wb')
    pickle.dump(artifact, pickle_file)
    pickle_file.close()
dump_artifact(overall_queries, "rap_queries_snake")
...
if args.csv_path:
    ...
    if os.path.exists(file_name):
        ...
        df_res = df_res._append(dfprev, sort=False)
