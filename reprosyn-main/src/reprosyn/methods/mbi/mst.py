...
def mst(data, epsilon, delta, rows):
    ...
    return undo_compress_fn(synth), est.cliques
...
def domain_from_metadata(metadata: list[dict]):
    ...
    ----------
    metadata : list[dict]
        metadata, see `Data Format <https://privacy-sdg-.readthedocs.io/en/latest/dataset-schema.html>`_
...
class MST(PipelineBase):
    ...
    def postprocess(self):
        ...
        self.cliques = self.output[1]
        self.output = decode_ordinal(self.output[0].df, self.encoders)
